...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        if (first == null) {
            ...
            if (pd == null) {
                pd = new ProbabilityDistribution<String>();
                pd.record(second);
                chain.put(first, pd);
            } else {
                pd.record(second);
                chain.replace(first, pd);
            }
        }
    ...
    public void train(Iterator<String> sentence) {
        if (sentence == null) {
            ...
            if (sentence.hasNext()) {
                String snd = sentence.next();
                startWords.record(snd);
    ...
    public String next() {
        if (hasNext()) {
            String temp = current;
            ProbabilityDistribution<String> pd = chain.get(current);
            if (pd == null) {
                current = null;
            } else {
                current = pd.pick(ng);
