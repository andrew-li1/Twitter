...
public class TweetParser {
    ...
    static List<String> csvFileToTweets(String pathToCSVFile, int tweetColumn) {
        ...
        Iterator<String> it = new FileLineIterator(pathToCSVFile);
        while (it.hasNext()) {
            String i = it.next();
            String str = extractColumn(i, tweetColumn);
            if (str != null) {
                result.add(str);
            }
        } 
    ...
    static List<String> parseAndCleanSentence(String sentence) {
        ...
        for (int i = 0; i < strArr.length; i++) {
            String cleaned = cleanWord(strArr[i]);
            if (cleaned != null) {
                result.add(cleaned);
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        List<String> splitted = sentenceSplit(removeURLs(tweet));
        List<List<String>> cleaned = new LinkedList<List<String>>();
        for (String s : splitted) {
            List<String> sNew = parseAndCleanSentence(s);
            if (!(sNew.isEmpty())) {
                cleaned.add(sNew);
            }
        }
    ...
    public static List<List<String>> csvFileToTrainingData(String pathToCSVFile, int tweetColumn) {
        ...
        for (String t : tweets) {
            trainingData.addAll(parseAndCleanTweet(t));
        }
        return trainingData;
    }
