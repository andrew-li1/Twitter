...
public class TwitterBot {
    ...
    public TwitterBot(String csvFile, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        List<List<String>> sentences = TweetParser.csvFileToTrainingData(csvFile, tweetColumn);
    ...
    public void writeStringsToFile(List<String> stringsToWrite, String filePath, boolean append) {
        ...
        Iterator<String> iter = stringsToWrite.iterator();
        try {
            while (iter.hasNext()) {
                bw.write(iter.next());
                bw.newLine();
    ...
    public String generateTweet(int length) {
        ...
        while (tweet.length() <= length) {
            if (mc.hasNext()) {
                tweet = tweet.concat(mc.next() + " ");
            } else {
                if (tweet.length() == length) {
                    break;
                }
                tweet = tweet.substring(0, tweet.length() - 1);
                tweet = tweet.concat(randomPunctuation() + " ");
                mc.reset();
            }
        }
        tweet = tweet.substring(0, tweet.length() - 1);
